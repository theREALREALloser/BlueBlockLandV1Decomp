...
else if (state != 68 || !grounded)
    audio_stop_sound(sfx_mach1);

if ((state == 69 || sprite_index == spr_player_machslideboost) && !audio_is_playing(sfx_mach2))
    scr_soundeffect(sfx_mach2);
else if (state != 69 && sprite_index != spr_player_machslideboost)
    audio_stop_sound(sfx_mach2);

if ((state == 89 || sprite_index == spr_player_machslideboost3) && !audio_is_playing(sfx_mach3))
    scr_soundeffect(sfx_mach3);
else if (state != 89 && sprite_index != spr_player_machslideboost3)
    audio_stop_sound(sfx_mach3);

if (state == 36 && !audio_is_playing(sfx_machroll))
...
if (state == 54 && sprite_index == spr_deathend && y > (room_height + 400))
    game_restart();

if (!instance_exists(baddiegrabbedID) && (state == 45 || (state == 42 && sprite_index != spr_piledriverland) || state == 9))
    state = 0;
...
if (state == 89 || state == 1 || state == 5 || state == 6 || state == 8 || state == 43 || state == 34 || state == 62 || state == 36 || state == 39 || state == 9 || (state == 42 && sprite_index == spr_piledriver) || state == 23 || state == 24 || state == 17 || state == 14 || state == 12 || state == 10)
    instakillmove = 1;
else
    instakillmove = 0;
...
if ((state == 89 || state == 69 || state == 8 || state == 36 || state == 9 || state == 21 || state == 70) && !instance_exists(obj_mach3effect))
{
    toomuchalarm1 = 6;
...
if (toomuchalarm1 > 0)
{
    ...
    if (toomuchalarm1 <= 0 && (state == 89 || state == 8 || state == 69 || state == 9 || state == 70 || state == 36 || state == 21 || (state == 32 && mach2 >= 100)))
    {
        instance_create(x, y, obj_mach3effect);
        toomuchalarm1 = 6;
    }
...
if (state != 71 && state != 1 && state != 5 && state != 65 && state != 14 && (state != 38 && sprite_index != spr_player_crouchshoot) && state != 64 && state != 32 && state != 36 && state != 72 && state != 67 && state != 66)
